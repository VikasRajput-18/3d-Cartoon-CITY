...
function storageKey(uid) { return 'clu3d_u_' + uid }
...
export const useStore = create((set, get) => ({
  ...
  initForUser: (uid, clerkUser) => {
    ...
    // First time — seed name from Clerk profile
    const name = clerkDisplayName(clerkUser)
    set(s => ({ avatar: { ...s.avatar, name }, isInitialized: true }))
  },

  saveForUser: (uid) => {
    const { avatar, stats, wallet, isOnboarded } = get()
    localStorage.setItem(storageKey(uid), JSON.stringify({ avatar, stats, wallet, isOnboarded }))
  },
  ...
  // Onboarding
  isOnboarded: false,
  completeOnboarding: (avatarPartial) => set(s => ({
    isOnboarded: true,
    avatar: { ...s.avatar, ...avatarPartial },
  })),
